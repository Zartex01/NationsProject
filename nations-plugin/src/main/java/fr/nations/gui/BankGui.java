...
public class BankGui {
    ...
    public Inventory build() {
        ...
        boolean canWithdraw   = member != null && member.canManageBank();

        inv.setItem(11, isMember
            ? GuiUtil.createItem(Material.HOPPER,
                "&aDepôt",
                "&7Déposer de votre argent",
                "&7dans la banque de la nation",
                "",
                "&eCliquer pour déposer")
            : GuiUtil.createItem(Material.BARRIER,
                "&cDepôt non autorisé",
                "&7Vous n'êtes pas membre",
                "&7de cette nation"));
        ...
        if (canWithdraw) {
            ...
            inv.setItem(15, GuiUtil.createItem(Material.BARRIER,
                ...
                "&7Votre rôle ne dispose pas",
                "&7de la permission de retrait"
            ));
        }
    ...
    public void handleClick(InventoryClickEvent event) {
        ...
        if (slot == 11) {
            ...
            if (clickMember == null) {
                MessageUtil.sendError(player, "Vous n'êtes pas membre de cette nation.");
                return;
            }
            player.closeInventory();
            MessageUtil.send(player, "&7Entrez le montant à &aDeposer &7dans le chat: (ex: §e100§7)");
            GuiManager.setPendingAction(player.getUniqueId(), "bank_deposit:" + nation.getId());
        } else if (slot == 15) {
            NationMember clickMember = nation.getMember(player.getUniqueId());
            if (clickMember == null || !clickMember.canManageBank()) {
                MessageUtil.sendError(player, "Votre rôle ne dispose pas de la permission de retrait.");
                return;
            }
            player.closeInventory();
